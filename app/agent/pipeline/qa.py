...
async def agent_qa_review(task_gid: str, task: dict, run: dict,
    ...
    try:
        ...
        prompt = (
            ...
            f"IMPORTANT: The diff only shows changed lines (+/-). A variable used in a `+` line may be "
            f"defined in: (a) unchanged code above/below the hunk, (b) a function parameter "
            f"(e.g. `function foo($bar)` — `$bar` is always defined inside that function), "
            f"(c) a class property, or (d) code earlier in the same function that is not in the diff. "
            f"Do NOT flag 'undefined variable' unless you can see the variable is introduced AND used "
            f"exclusively within `+` lines with NO definition anywhere in those same `+` lines. "
            f"When in doubt, assume the variable IS defined — the diff does not show the full file.\n"
