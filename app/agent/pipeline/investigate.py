...
async def agent_investigate(task_gid: str, context: str, run: dict) -> Optional[str]:
    ...
    try:
        ...
        system = (
            ...
            "3. **Check cross-project impact** — does this touch shared models, APIs, migrations, or data that "
            "lives in another repo? A feature can need changes in BOTH backends (e.g. the data is produced/stored "
            "in one backend and displayed in the other). Grep the OTHER repos for the relevant model/field/endpoint "
            "before concluding it's single-repo.\n"
            ...
            "If this task REQUIRES changes in a repo that is NOT already one of YOUR WORKTREEs above, include "
            "this line at the very end, using the exact repo-id(s) from the 'Available repositories' list:\n\n"
            "ADDITIONAL_REPOS: repo-id-1, repo-id-2\n\n"
            "Use the repo IDs verbatim (e.g. back-clientes, back-proveedores) — not project names or paths. "
            "Only request repos where code changes are actually needed; if none, omit the line entirely. "
